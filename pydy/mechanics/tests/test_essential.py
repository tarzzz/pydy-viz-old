...
def test_ang_vel():
    ...
    # Test for Addition theorem of angular velocities
    # It states that ...
    #^N omega ^A = (^N omega ^A1) + (^A1 omega ^A2) + ...
    #           ... (^An-1 omega ^An)+ (^An omega ^A)
    # A1, A2 etc. are Auxillary ReferenceFrames

    assert A.ang_vel_in(N) == B.ang_vel_in(N) + C.ang_vel_in(B) + A.ang_vel_in(C)
